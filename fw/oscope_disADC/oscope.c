...
static uint32_t adc_state;
static uint32_t state;
static uint16_t leds;
//static uint8_t DMA_l = 60;
#define DMA_l 120
static uint16_t adc_samples[120*62/2];
...
static void adc_setup(void)
{
    ...
    ADC1_CFGR = ADC_CFGR_CONT | ADC_CFGR_EXTEN_RISING_EDGE | ADC_CFGR_EXTSEL_EVENT_4; //CONTINOUS MODE
    //ADC1_CFGR = ADC_CFGR_CONT | ADC_CFGR_DMAEN | ADC_CFGR_EXTEN_RISING_EDGE | ADC_CFGR_EXTSEL_EVENT_4; //CONTINOUS MODE DMA
...
static void timer_setup(void) {
    ...
    TIM3_CNT = 0;
    TIM3_PSC = 0;
    TIM3_ARR = 1000; // 0xFFFF;
    TIM3_CCR1 = 500; //0x8000;
...
static void cdcacm_data_tx_cb(usbd_device *usbd_dev, uint8_t ep) {
    ...
    if (state == DMA_l/2){
        ...
        //TIM3_SMCR |= TIM_SMCR_SMS_TM; //trigger mode
